...
> 對象 spec：`specs/Task6.spec.md`。撰寫當下 sw.js 實際 `CACHE_VERSION = 'v12'` → 本任務 bump **v13**（若前方插隊自動吸收）。
...
- 掛載物件加一行 `ocr: ocr,`，並移除既有預留註解行（api.js:170 `// Task6 在此追加：...`）——**這是全檔唯一允許刪除的一行**。
...
`camera-tab.js` 載入行插在 `translate-tab.js` 之後、`coupon-viewer.js` 之前（index.html:142–143 之間，即現有 comment「Task4 功能模組」之前）。wrap 是載入時捕獲當時的 `App.showTab`，故鏈序＝載入順序**反向**：
...
| App 進背景/鎖屏 | `document.addEventListener('visibilitychange')`：`document.hidden` 為 true 且 track 存活 → 停 | **SA 定案：要做**。iOS 背景本會 mute/end camera stream，但主動停讓狀態機確定、綠點熄滅 |
...
| 離線快取 | 全站 / `sw.js` | v12→v13＋PRECACHE +`./js/camera-tab.js`（38 筆）；bump 付 16 券圖重下成本（已知，行前 wifi 可接受） | ✅（冷 install 驗法照 SYSTEM_MAP 紀律） |
| App shell | 全站 / `index.html` | `#tab-camera` 佔位換實體＋插一行 script；section 本體/分頁 id/其餘載入順序不動 | ✅（五分頁可切、載入無 console error） |
...
- [ ] Non-scope 檔零 diff：translate-tab / recorder / tts / bigtext / app / coupon-viewer / coupons-tab / phrases* / trip* / import-data / tripdata / config* / manifest
...
- [ ] sw.js：v13、PRECACHE 38 筆含 `./js/camera-tab.js`；冷 install 離線驗法
...
人工補充區已更新：wrap 鏈條目補「Task6 立案後擴為四層」與 camera 層守門條件；雙 overlay 預警對 Task6 解除（取景為分頁內容非 overlay）。實作完成、PM 閉環時再把檔案結構表的 camera-tab.js/sw.js v13 落成現況（本 repo 無生成腳本，全人工維護）。
