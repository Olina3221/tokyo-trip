...
| **1（下一個）** | **Task10** | **行程頁字級統一（五階 type scale）＋單日細節下鑽（總覽⇄單日視圖切換）（Olina 實機回饋）。字級以 Task8 淺色主題為基礎** | 否 | 🔨 spec 完成（`Task10.spec.md`）。**Task8 已閉環（2026-07-11），依賴閘解除**——`Task10.ready` 已在 specs/，SA 可開工 |
...
閉環註記（2026-07-11）：**Task8 已正式閉環**，Task9/Task10 的「等 Task8」依賴閘皆已解除——Task10 為下一個（`Task10.ready` 待 SA），Task9 仍等 Olina 句子清單（Task8 側已無阻擋）。
