...
```
tokyo-trip/
├── index.html              ✅ Task1–3,8,5  App shell：iOS meta、分頁容器、載入所有 js（Task2 加 tts/bigtext/phrases-tab、Task3 加 import-data/trip-tab、Task5 加 api/translate-tab（插 trip-tab.js 後、coupon-viewer.js 前），順序定死；Task8 狀態列 black-translucent→default、theme-color→#F5F6F8）
├── css/
...
│   ├── translate-tab.js    ✅ Task5  翻譯分頁（registerTab('translate',{onShow}) 冪等；中⇄日方向切換（localStorage `tokyotrip.translateDir`，壞值 fallback zh2ja、try/catch 降級）；輸入上限 500（text.length）、防連點 _isTranslating、方向切換清空結果；五種錯誤訊息對應 App.api.ErrorCode；中→日結果接 showBigText({ja,zh})/App.speak/複製，日→中僅複製；DOM class 見 Task5.api.md）
│   └── api.js              ✅ Task5  Google API 呼叫層（`window.App.api = { ErrorCode, translate }`；POST-only `_postJson`——GET 是破口；金鑰/傳輸/端點三層分離，金鑰層可選鏈讀 APP_CONFIG；ErrorCode 枚舉 NO_KEY/OFFLINE/HTTP_403/HTTP_429/HTTP_OTHER；不碰 DOM/localStorage/TTS；Task6 Vision OCR 只在端點層追加、不動上兩層——重用邊界見 Task5.api.md）
├── sw.js                   ✅ Task1–4,8,11,9,10,5  Service Worker：cache-first 離線快取（必須在根目錄，scope 才涵蓋全站；現況 CACHE_VERSION='v10'（Task5 bump），PRECACHE_URLS 含 Task2 三新檔＋Task3 兩新檔＋Task4 兩 js＋16 張券圖＋Task5 三筆（config.js/api.js/translate-tab.js）；Task5 起 A3 config.js network-only 特例整段刪除，method!==GET 直通檢查保留（POST 天然不快取））
