...
│   ├── version.js          ✅ Task14  版號單一來源（window.APP_VERSION／APP_VERSION_DATE 兩常數，現況 'v13'／'07/12'；APP_VERSION 與 sw.js CACHE_VERSION 逐字元相等＝QA 機械閘；載入順序全站最前；純常數零依賴，缺載 no-op 不壞頁）
...
│   ├── translate-tab.js    ✅ Task5,12,13  翻譯分頁（registerTab('translate',{onShow}) 冪等；Task12 加「文字/對話」segmented 雙模式（localStorage `tokyotrip.translateMode`，預設與壞值 fallback 皆 'talk'；Task5 文字模式 16 個既有單元零 diff）；文字模式：中⇄日方向切換（`tokyotrip.translateDir` 壞值 fallback zh2ja）、輸入上限 500、防連點、五種錯誤訊息、中→日接 showBigText/speak/複製、日→中僅複製；對話模式：狀態機 idle→recording→recognizing→translating→idle、雙麥克風鈕（中/日）、60 秒自動停、`_abortTalk()` 統一清理（timer+abort+cancel+回 idle）、氣泡歷史純記憶體上限 50（陣列+DOM 同步裁剪，禁 localStorage）、**雙向自動播（zh→ja-JP／ja→zh-TW，G4 守門共用：translate 為當前分頁 && 模式 'talk'——Task13 翻轉 Task12 spec §3「日→中不自動播」）**、氣泡動作鈕重播/大字；wrap App.showTab（三層鏈之中層，見人工補充區）；DOM class 見 Task5.api.md/Task12.api.md）
...
├── sw.js                   ✅ Task1–4,8,11,9,10,5,12,13,14  Service Worker：cache-first 離線快取（必須在根目錄，scope 才涵蓋全站；現況 CACHE_VERSION='v13'（Task14 bump，與 version.js APP_VERSION 逐字元相等），PRECACHE_URLS 38 筆＝Task2 三新檔＋Task3 兩新檔＋Task4 兩 js＋16 張券圖＋Task5 三筆（config.js/api.js/translate-tab.js）＋Task12 一筆（recorder.js）＋Task14 一筆（version.js）；Task5 起 A3 config.js network-only 特例整段刪除，method!==GET 直通檢查保留（POST 天然不快取，STT 同受保護）；Task14 檔頭 SOP 註解改寫為兩檔三行紀律，install skipWaiting／activate clients.claim／fetch handler 零 diff）
...
| 翻譯 | 文字/對話雙模式：文字＝中⇄日輸入翻譯 → 大字/播音；對話＝語音對話即時互譯（錄音→STT→翻譯→**雙向自動 TTS**→氣泡歷史） | Cloud Translation + Cloud Speech-to-Text | Task5 ✅ + Task12 ✅ + Task13 ✅ |
