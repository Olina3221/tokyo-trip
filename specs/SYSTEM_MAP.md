...
├── sw.js                   ✅ Task1–4,8,11,9,10,5,12,13  Service Worker：cache-first 離線快取（必須在根目錄，scope 才涵蓋全站；現況 CACHE_VERSION='v12'（Task13 bump），PRECACHE_URLS 37 筆＝Task2 三新檔＋Task3 兩新檔＋Task4 兩 js＋16 張券圖＋Task5 三筆（config.js/api.js/translate-tab.js）＋Task12 一筆（recorder.js）；Task5 起 A3 config.js network-only 特例整段刪除，method!==GET 直通檢查保留（POST 天然不快取，STT 同受保護））
