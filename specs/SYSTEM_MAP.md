...
```
tokyo-trip/
├── index.html              ✅ Task1–3,8,5,12,14  App shell：iOS meta、分頁容器、載入所有 js（Task2 加 tts/bigtext/phrases-tab、Task3 加 import-data/trip-tab、Task5 加 api/translate-tab（插 trip-tab.js 後、coupon-viewer.js 前）、Task12 加 recorder.js（插 api.js 後、translate-tab.js 前，最終 api → recorder → translate-tab），順序定死；Task8 狀態列 black-translucent→default、theme-color→#F5F6F8；Task14 加 version.js 於**所有 script 最前**（A6 注解改寫：version → config → phrases → tripdata → app → 功能模組）＋兩 DOM 元素 #app-version／button#update-toast[hidden]（section 外掛 body，HTML 不寫死版號字串——文字由 app.js 填））
├── css/
...
├── js/
│   ├── app.js              ✅ Task1,14  分頁切換框架、SW 註冊、共用工具（Task2 未改本體，showTab 被 bigtext.js 外掛 wrap；Task14 SW 註冊段擴充落地：`updateViaCache:'none'`、`hadController` register 前快照、visibilitychange(visible)/pageshow → `_triggerUpdate()`（S2 null-check＋S1 promise .catch）、controllerchange＋updatefound→activated 雙路 → `#update-toast`（`_updateShown` 記憶體 flag、首裝不彈）、toast click＝全 repo 唯一 `location.reload`、DOMContentLoaded 填 #app-version（缺元素/缺常數 no-op）；registerTab/showTab/TAB_IDS/lastTab 零 diff）
│   ├── version.js          ✅ Task14,15  版號單一來源（window.APP_VERSION／APP_VERSION_DATE 兩常數，現況 'v14'／'07/12'（Task15 bump）；APP_VERSION 與 sw.js CACHE_VERSION 逐字元相等＝QA 機械閘；載入順序全站最前；純常數零依賴，缺載 no-op 不壞頁）
...
│   ├── translate-tab.js    ✅ Task5,12,13,15  翻譯分頁（registerTab('translate',{onShow}) 冪等；Task12 加「文字/對話」segmented 雙模式（localStorage `tokyotrip.translateMode`，預設與壞值 fallback 皆 'talk'；Task5 文字模式 16 個既有單元零 diff）；文字模式：中⇄日方向切換（`tokyotrip.translateDir` 壞值 fallback zh2ja）、輸入上限 500、防連點、五種錯誤訊息、中→日接 showBigText/speak/複製、日→中僅複製；對話模式（Task15 起面對面雙側版面）：狀態機 idle→recording→recognizing→translating→idle、每側各自麥克風鈕（上半 `.talk-side-ja` 由 CSS rotate 180° 給對面日方、下半 `.talk-side-zh` 正向）、60 秒自動停、`_abortTalk()` 統一清理（timer+abort+cancel+回 idle）、顯示路由＝辨識原文寫說話側 `.talk-side-orig`／譯文寫聽話側 `.talk-side-result`（zh 說→ja 側、ja 說→zh 側；覆蓋式每側只留最近一則，`_side` 記憶體雙槽 {zh:{result},ja:{result,origZh}}；氣泡歷史 `_bubbles`/`_appendBubble`/MAX_BUBBLES/`.talk-history` 已退場，禁 localStorage 不變）、錯誤路由＝API 錯誤→下半 zh（機主處理）／空辨識→說話側／NOT_SUPPORTED→雙 mic disabled、**雙向自動播（zh→ja-JP／ja→zh-TW，G4 守門共用：translate 為當前分頁 && 模式 'talk'——Task13 翻轉 Task12 spec §3「日→中不自動播」；Task15 G4 四行逐字保留，顯示寫入在守門外無條件執行）**、側動作鈕重播/大字（無結果時 disabled 非隱藏；ja 側大字 {ja,zh}、zh 側 {ja,lang:'zh-TW'}——契約沿 Task12.api.md）；wrap App.showTab（三層鏈之中層，見人工補充區）；DOM class 見 Task5.api.md/Task15.api.md（Task12.api.md 氣泡 DOM 節自 Task15 起為歷史存檔））
...
├── sw.js                   ✅ Task1–4,8,11,9,10,5,12,13,14,15  Service Worker：cache-first 離線快取（必須在根目錄，scope 才涵蓋全站；現況 CACHE_VERSION='v14'（Task15 bump，與 version.js APP_VERSION 逐字元相等），PRECACHE_URLS 38 筆＝Task2 三新檔＋Task3 兩新檔＋Task4 兩 js＋16 張券圖＋Task5 三筆（config.js/api.js/translate-tab.js）＋Task12 一筆（recorder.js）＋Task14 一筆（version.js）；Task5 起 A3 config.js network-only 特例整段刪除，method!==GET 直通檢查保留（POST 天然不快取，STT 同受保護）；Task14 檔頭 SOP 註解改寫為兩檔三行紀律，install skipWaiting／activate clients.claim／fetch handler 零 diff）
...
| 翻譯 | 文字/對話雙模式：文字＝中⇄日輸入翻譯 → 大字/播音；對話＝語音對話即時互譯（錄音→STT→翻譯→**雙向自動 TTS**→**面對面雙側槽顯示**（上半旋轉 180° 給日方、譯文寫聽話側 ≥28px、覆蓋式每側最近一則）） | Cloud Translation + Cloud Speech-to-Text | Task5 ✅ + Task12 ✅ + Task13 ✅ + Task15 ✅ |
...
- **bump SOP＝兩檔三行（Task14 起永續紀律，取代舊「只改 sw.js 兩常數」）**：任何 bump 必須同步改 `js/version.js` 的 `APP_VERSION`（與 `CACHE_VERSION` **逐字元相等**，QA 每輪機械閘）＋`APP_VERSION_DATE`（`MM/DD`，bump 當天台灣時區）＋`sw.js` 的 `CACHE_VERSION`。版號與日期只存在 version.js 一處（HTML/CSS/其他 js 含註解不得寫死版號字串）；畫面徽章 `vN · MM/DD` 由 app.js 填 `#app-version`。採雙常數而非 importScripts 單一來源＝定案（sw.js 主檔 byte-diff 是最可靠更新訊號，見 Task14.spec.md A1）。Task6（順延中，spec 寫「開工時實際值+1」自動吸收）與之後所有 Task 一體適用。
